...
### Added
- **Landing page (`landing/`)**: neo-brutalist marketing site built with Astro (static, zero client JS by default). Design tokens ported 1:1 from `src/theme.ts` into `src/styles/tokens.css` (ink/lime, 2px borders, zero-blur offset shadows, press-drop interaction). Web recreations of the app's signature objects — `VoiceNoteCard`, `Waveform` (same shape fn as `WaveformVisualizer`), floating `TabBar` (FEED/NOTES/ME) — plus `PhoneFrame` mocks that show a screenshot or a labelled placeholder. Sections: stacked-card hero with lime blob, mono-caps ink ticker, 3-step "how it works", six-reaction brand wall, live-feed + DM feature blocks, manifesto band, download CTA. Fonts (Bricolage Grotesque / Hanken Grotesk / JetBrains Mono) via Google Fonts. `LANDING_STYLE_GUIDE.md` moved from repo root into `landing/`. Two screenshot placeholders await real captures (see `landing/README.md`).
